...
exports.createOrder = async (req, res) => {
    const { id, consumerId , productId ,  loggedUserType } = req.body;
    console.log('1');

    if ( !id || !consumerId || !productId || !loggedUserType) {
        return apiResponse.badRequest(res);
    }
    console.log('2');
    ...
    const modelRes = await productModel.createOrder({productId, consumerId, id });
    return apiResponse.send(res, modelRes);
};


exports.isDelivered = async (req, res) => {
    const { id , productId ,  loggedUserType } = req.body;
    console.log('1');

    if ( !id || !productId || !loggedUserType) {
        return apiResponse.badRequest(res);
    }
    console.log('2');
